...
export default class ClientForm extends React.Component {
    ...
    async detectAction() {
        ...
        if(action === "update") {
            ...
            const response = await request.GET(`${api}/clients/getone/${id}`);
            const values = response.data;
            this.setState({values, title: "Actualizar Cliente"});
        }
    }
    ...
    async handleSubmit() {
        ...
        if(action === "create") {
            values.user_id = 0;
            delete values.id;
            const response  =  await request.POST(api + "/clients/add", {data: values});
            (response.error) ? alert(response.msg) : this.props.navigation.goBack();
        } else {
            values._id = values.id;
            delete values._id;
            const response  =  await request.POST(api + "/clients/update", {data: values});
            (response.error) ? alert(response.msg) : this.props.navigation.goBack();
        }
    }
